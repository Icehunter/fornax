 ...
 *
 * <p>{@link VulkanMetalInterop} used to raise these as bare {@code IllegalStateException} --
 * {@code "interop timeline wait failed"}, {@code "vkEndCommandBuffer failed"},
 * {@code "interop fence timeout"}, {@code "Metal command buffer/blit encoder nil"} -- which no
 * catch could distinguish from an ordinary logic error without string-matching the message. A
 * named type is what {@link GpuFatalErrors#rethrowIfFatal} needs to tell the two apart.
 *
