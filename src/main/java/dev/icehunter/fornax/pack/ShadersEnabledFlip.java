 ...
 * RuntimeShaderPack}'s published vanilla-shader override here without this method's own
 * clear/republish pairing -- without it, a pack disabled this way keeps serving a vanilla
 * core-shader override (e.g. the curved lightmap) until restart.
 *
 ...
 * mismatch (see {@link RendererReload}'s own doc comment). The renderer reload below
...
public final class ShadersEnabledFlip {
     ...
     * renderer reload onto the override future's completion -- the same sequence {@code
     * ShaderPacksScreen.applyChanges}'s master-toggle-only branch needs for its own case.
     */
