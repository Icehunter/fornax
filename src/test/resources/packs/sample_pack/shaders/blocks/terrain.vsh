...
// Sample-pack terrain vertex stage. Decodes Fornax's own vertex format (chunk_vertex.glsl), places the
...
// fornax_runtime, not fornax: chunk_vertex.glsl is pack-local (this repo's shaders/include/), served
// by Fornax's runtime shader pack under its own namespace. Only globals.glsl above is the engine
// built-in that still ships inside the fornax jar.
