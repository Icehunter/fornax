...
// Sample-pack terrain fragment stage. Two output shapes share this file, selected by USE_DEFERRED:
...
//
// bumpStrength/aoStrength are pack runtime options (see the annotated #defines below), but this is a
// geometry-slot ("terrain") shader, not a fullscreen post pass -- it never goes through
// FullscreenPassRunner's generic u_PackOptions bind group (GraphRunner treats GEOMETRY passes as a
// pure no-op; Sodium's own chunk-renderer bind group is untouched by the pack-option system). The
// annotations below exist purely so OptionScanner/screens.toml can discover these two options (for
// defaults + the settings UI); the actual value delivery still rides the small, pre-existing
// u_PbrSettings uniform block, unchanged in shape -- only its Java-side data source changed (see
// UniformBufferManagerMixin.updatePbrSettings(), now reading GraphRunner.optionsBuffer() instead of
// the deleted FornaxSettings.bumpStrength/aoStrength fields).
...
void main() {
    ...
    // is lit identically to a flat face. Live-tunable via the pack's Lighting settings.
